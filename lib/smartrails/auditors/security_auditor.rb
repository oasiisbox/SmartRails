  ...
  module Auditors
    class SecurityAuditor < BaseAuditor
      ...
      def check_csrf_protection
        ...
        unless app_controller.include?('protect_from_forgery')
          add_issue(
      ...
      def check_force_ssl
        ...
        unless production_rb.include?('config.force_ssl = true')
          add_issue(
      ...
      def check_secure_headers
        unless gemfile_path.read.include?('secure_headers')
          add_issue(
      ...
      def check_strong_parameters
        Dir.glob(app_dir.join('controllers/**/*.rb')).each do |controller_file|
          ...
          if content.include?('params[') && !content.include?('params.require')
            add_issue(
      ...
      def check_secrets_management
        ...
        unless file_exists?('config/credentials.yml.enc', 'config/master.key')
          add_issue(
            type: 'Secrets Management',
      ...
      def check_authentication
        ...
        unless auth_gems.any? { |gem| gemfile_content.include?(gem) }
          add_issue(
            type: 'Authentication',
